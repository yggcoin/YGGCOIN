...
#if defined(NDEBUG)
#error "Ygg cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uygg) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight >= 2 && nHeight <= 2160) {
        nSubsidy = 0.25 * COIN;
    }
    else if (nHeight >= 2161 && nHeight <= 6480) {
        nSubsidy = 1 * COIN;
    }
    else if (nHeight >= 6481 && nHeight <= 9360) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight >= 9361 && nHeight <= 12240) {
      nSubsidy = 3 * COIN;
    }
    else if (nHeight >= 12241 && nHeight <= 15120) {
      nSubsidy = 4 * COIN;
    }
    else if (nHeight >= 15121 && nHeight <= 16560) {
      nSubsidy = 5 * COIN;
    }
    else if (nHeight >= 16561 && nHeight <= 18000) {
      nSubsidy = 6 * COIN;
    }
    else if (nHeight >= 18001 && nHeight <= 19440) {
      nSubsidy = 8 * COIN;
    }
    else if (nHeight >= 19441 && nHeight <= 20160) {
      nSubsidy = 10 * COIN;
    }
    else if (nHeight >= 20161 && nHeight <= 20880) {
      nSubsidy = 20 * COIN;
    } else {
      nSubsidy = 5 * COIN;
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // YGG: We use certain sporks during IBD, so check to see if they are
